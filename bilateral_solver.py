...
__all__ = ['apply_bilateral_solver']
...
def apply_bilateral_solver(t: torch.Tensor, r: torch.Tensor, c: torch.Tensor = None, grid_params={}, bs_params={}):
    ''' Applies bilateral solver on target `t` using confidence `c` and reference `r`.

    Args:
        t (torch.Tensor): Target to filter (1, W, H)
        r (torch.Tensor): Reference image  (1, W, H)
        c (torch.Tensor): Confidence for target (Defaults to target image `t`) (1, W, H)
        grid_params (dict, optional): Grid parameters for bilateral solver. May include `sigma_luma`, `sigma_chroma` and `sigma_spatial`. Defaults to {}.
        bs_params (dict, optional): Bilateral solver parameters. May inlcude `lam`, `A_diag_min`, `cg_tol` and `cg_maxiter`. Defaults to {}.
    
    Returns:
        torch.Tensor: Bilaterally solved target (1, W, H)
    '''
    gp = {**grid_params_default, **grid_params}
    bs = {**bs_params_default, **bs_params}
    dev, typ = t.device, t.dtype
    shap = t.shape[-2:]
    t = t.cpu().permute(1,2,0).numpy().squeeze(-1).reshape(-1, 1)
    r = r.cpu().permute(1,2,0).numpy()
    if c is None:
        c = t
        c = np.ones(shap).reshape(-1,1) * 0.999
    else:
        c = c.cpu().permute(1,2,0).numpy().astype(np.double).reshape(-1,1)

    grid = BilateralGrid(r, **gp)
    # tc_filt = grid.filter(t * c)
    # c_filt = grid.filter(c)
    # out_filt = (tc_filt / c_filt).reshape(t.shape[:2])
    out = BilateralSolver(grid, bs).solve(t, c).reshape(*shap)

    binary_solver = ndimage.binary_fill_holes(out>0.5)
    ...
    try : 
        ...
        binary_solver = np.ones(shap, dtype=bool)

    return torch.from_numpy(binary_solver).to(torch.float32).to(dev).squeeze(), torch.from_numpy(out).to(torch.float32).to(dev).squeeze()
