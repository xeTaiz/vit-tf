...
def main():
    ...
    parser.add_argument('--numpy', action='store_true', help='Save as numpy files')
    ...
    spheret = noise_clamp(sphere_thickness(pos, 0.5, 0.1))
    spheref = noise_clamp(sphere_filled(pos, 0.5))
    torust = noise_clamp(torus_thickness(pos, 0.5, 0.2, 0.1))
    torusf = noise_clamp(torus_filled(pos, 0.5, 0.2))

    save_vol(spheret, 'sphere_thickness')
    save_vol(spheref, 'sphere_filled')
    save_vol(torust, 'torus_thickness')
    save_vol(torusf, 'torus_filled')
